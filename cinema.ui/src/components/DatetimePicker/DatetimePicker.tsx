...
import { enUS, type Locale, pl } from 'date-fns/locale';
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, Clock } from 'lucide-react';
import { DayPicker } from 'react-day-picker';

import { baseButtonVariants, Button, ButtonVariant } from 'Components/Button';
...
function Calendar({
  ...
  const MONTHS = React.useMemo(() => {
    let locale: Pick<Locale, 'options' | 'localize' | 'formatLong'> = pl;
        ...
        MonthCaption: ({ calendarMonth }) => {
          return (
            <div className="inline-flex gap-2">
              ...
              >
                <CalendarSelectTrigger className="w-fit gap-1 border-none p-0 focus:bg-accent focus:text-accent-foreground">
                  <CalendarSelectValue />
                </CalendarSelectTrigger>
                <CalendarSelectContent>
                  {MONTHS.map(month => (
                    <CalendarSelectItem key={month.value} value={month.value.toString()}>
                      {month.label}
                    </CalendarSelectItem>
                  ))}
                </CalendarSelectContent>
              ...
              >
                ...
                <CalendarSelectContent>
                  {YEARS.map(year => (
                    <CalendarSelectItem key={year.value} value={year.value.toString()}>
                      {year.label}
                    </CalendarSelectItem>
                  ))}
                </CalendarSelectContent>
    ...
    return (
      <div className="flex h-10 items-center">
        ...
        >
          ...
          <CalendarSelectContent>
            <CalendarSelectItem value="AM">AM</CalendarSelectItem>
            <CalendarSelectItem value="PM">PM</CalendarSelectItem>
          </CalendarSelectContent>
        </CalendarSelect>
      </div>
    );
